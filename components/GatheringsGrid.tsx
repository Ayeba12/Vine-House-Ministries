        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:col-span-8">
          {pillars.map((pillar, idx) => (
            <Reveal key={pillar.number} delay={idx * 0.06}>
              {/* Every field the editor fills in WordPress is on the tile: subtitle, title, number, when, where. */}
              <article className="flex h-full min-h-[240px] flex-col rounded-xl bg-surface-tint p-6 sm:p-7">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    {pillar.subtitle && <p className="eyebrow text-ink-muted">{pillar.subtitle}</p>}
                    <h3 className="font-anton scale-step-h5 mt-2 text-balance text-ink-strong">{pillar.title}</h3>
                  </div>
                  <span className="font-anton scale-step-h5 tabular-nums text-ink-muted">{pillar.number}</span>
                </div>
                <dl className="meta mt-auto grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 pt-8 text-ink-muted">
                  <dt>When</dt>
